...
class Import(NamedTuple):
    ...
    def __str__(self) -> str:
        return (
            f"{self.file_path or ''}:{self.line_number} " f"{'indented ' if self.indented else ''}{self.statement()}"
        )
...
def imports(
    ...
    for index, raw_line in indexed_input:
        ...
        for statement in statements:
            ...
            just_imports = [
                item.replace("{|", "{ ").replace("|}", " }") for item in _strip_syntax(import_string).split()
            ]
